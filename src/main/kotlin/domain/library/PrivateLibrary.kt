...
/**
 *
 * Private Library<br></br>
 * <br></br>
 * A Private Library is not a system Library, so it doesn't access the Account Role Object for any account checks.<br></br>
 * <br></br>
 ...
 * **`checkOut`** any **`Book`**.
 *  * *Note: A special case **`PrivateLibrary`** is an Orphan ****`PrivateLibrary` which
 * only allows a single Book of a specific id to be checked into/out of it. See below.*
 *
 * <br></br>
 * BOOP design notes for this **`PrivateLibrary`** class:<br></br>
 * <br></br>This is a system design **alternative** to:<br></br>
 *
...
class PrivateLibrary : Library, IUUID2 {
    private val isForOnlyOneBook // true = ORPHAN Private Library, false = normal Private Library
            : Boolean
    ...
    constructor(
        bookId: UUID2<Book>,
        @Suppress("unused") isForOnlyOneBook: Boolean,  // note: always true for this constructor
        context: Context
    ) : super(UUID2<Library>(bookId), context) // make the LibraryId match the BookId
    {
    ...
    override fun checkOutBookToUser(book: Book, user: User): Result<Book> {
        ...
        val firstBookId: UUID2<Book> = bookIds.toTypedArray()[0] as UUID2<Book> // there should only be 1 bookId
    ...
    override fun checkInBookFromUser(book: Book, user: User): Result<Book> {
        ...
        // todo extract info() to var
        if (info()!!.findAllKnownBookIds().isNotEmpty()) return Result.failure(Exception("Orphan Private Library can only check-in 1 Book from Users, bookId: " + book.id()))
        ...
        if (checkInResult.isFailure) return Result.failure(Exception("Failed to check-in Book from Private Library, bookId: " + book.id()))
